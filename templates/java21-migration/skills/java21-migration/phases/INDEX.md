...
| characterization (optional) | Baseline shows low coverage or high-risk behavior not locked by tests | baseline evidence, risk gaps, test inventory | focused characterization tests and Java 8 pass evidence | critical behavior gaps are reduced and remaining gaps are explicit | `Use the java21-migration skill and run optional characterization for high-risk gaps.` Then write status in `docs/migration-progress-checklist.md`. |
...
`baseline -> (optional characterization) -> planning -> implementation -> validation -> audit`
